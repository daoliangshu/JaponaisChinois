...
public class TextureHelper {
    public static int loadTexture(Context context, int resourceId){
        ...
        if (textureObjectIds[0] == 0) {
                Log.w("OPENGL", "Could not generate a new OpenGL texture object.");
            return 0;
        }
        ...
        if (bitmap == null) {
                Log.w("OPENGL", "Resource ID " + resourceId + " could not be decoded.");
    ...
    public static int loadTexture(Context context, Bitmap bm){
        if(bm == null)return 0;
        final int[] textureObjectIds = new int[1];
        glGenTextures(1, textureObjectIds, 0);
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        // Load the bitmap into the bound texture.
        texImage2D(GL_TEXTURE_2D, 0, bm, 0);
