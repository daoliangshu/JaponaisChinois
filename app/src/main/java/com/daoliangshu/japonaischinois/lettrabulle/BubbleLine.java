...
import com.daoliangshu.japonaischinois.Settings;
...
public class BubbleLine {
    ...
    public void setWord(String word) {
        ...
        this.guessLetterIndexes = BubbleLine.setRandomizedGuessIndexes(word);
        ...
        textPaint = new Paint();
        textPaint.setTextSize(2 * LB_Config.bubbleDiameter / 4);
    ...
    public void setLetter(int index) {
        ...
        textPaint = new Paint();
        textPaint.setTextSize(2 * LB_Config.bubbleDiameter / 4);
        textPaint.getTextBounds(word, index, index + 1, textBounds);
        offsetTextX = LB_Config.bubbleDiameter / 2 - (textBounds.width() / 2);
        offsetTextY = (int) (currentCanvas.getHeight() / (2 * (1 + lineCount)) -
                ((textPaint.descent() + textPaint.ascent()) / 2));
    ...
    public static ArrayList<Integer> setRandomizedGuessIndexes(String word) {
        ...
        int numberOfGuess = 1 + (int)((Settings.curEmptyRatio-1.0/word.length())* Math.abs(rand.nextInt() % word.length()) );
        ...
        for (int i = 0; i < numberOfGuess; i++) {
            ...
            } while (res.contains(newIndex) ||
                     word.charAt(newIndex) == ' ' ||
                     word.charAt(newIndex) == '」' ||
                    word.charAt(newIndex) == '「' ||
                    word.charAt(newIndex) == '。' ||
                    word.charAt(newIndex) == '～' ||
